...
This function (provide by Camden): from my understanding searches the page for a specific item and adds it to the cart, then proceeds to checkout
...
``` python 
First Iteration of the Bot find, and purchase functionalities curtiersy of Camden (and a job well done to offer)
while True:
    try:
        # get current url
